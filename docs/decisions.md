...
| 7 | **Seed data migration** | Wipe and re-seed, migration scripts, onCreate-only | `seedVersion` column + idempotent `onOpen` with `INSERT OR IGNORE` | Never touch user-created data (`isDefault = false`). Seed data runs on every launch using the raw `SupportSQLiteDatabase` (no deadlock risk). New app versions add rows with higher `seedVersion`. Safe, additive, and self-healing. |
