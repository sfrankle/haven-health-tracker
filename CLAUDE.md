...
Seed data uses `seedVersion` column for safe migrations across app updates. Delivered idempotently in `RoomDatabase.Callback.onOpen` via `INSERT OR IGNORE` on the raw `SupportSQLiteDatabase` — runs every launch, skips existing rows. See `docs/schema.sql` for full schema and seed data reference.
